...
const MedicineModal: React.FC<MedicineModalProps> = ({ isOpen, onClose, onSave, editingMedicine }) => {
  ...
  const handleSubmit = () => {
    if (formData.name && formData.dosage && formData.time && formData.stock !== undefined && formData.stock >= 0) {
      onSave(formData);
      onClose();
    }
  };
...
const MedicineTracker: React.FC = () => {
  ...
  const sortedMedicines = [...medicines].sort((a, b) => {
    const aValRaw: any = a[sortField];
    const bValRaw: any = b[sortField];

    // handle numeric stock
    let aVal: any = aValRaw;
    let bVal: any = bValRaw;

    if (sortField === 'stock') {
      aVal = Number(aValRaw);
      bVal = Number(bValRaw);
    } else if (sortField === 'addedAt') {
      aVal = new Date(aValRaw).getTime();
      bVal = new Date(bValRaw).getTime();
    } else {
      aVal = String(aValRaw).toLowerCase();
      bVal = String(bValRaw).toLowerCase();
    }
  ...
  const filteredMedicines = useMemo(() => {
    ...
    return sortedMedicines.filter((med) =>
      ...
      med.time.toLowerCase().includes(q) ||
      String(med.stock).includes(q)
    );
  }, [sortedMedicines, debouncedSearch]);
